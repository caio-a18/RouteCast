...
Routecast supports four travel modes: driving, walking, cycling, and public transit. Each mode uses a different average speed to calculate checkpoint arrival times, ensuring the forecast is accurate for how the user is actually travelling. Users can also save frequently used routes, stored locally on device using **SwiftData** and loadable instantly from a saved routes screen without re-entering origin and destination.
...
4.  **Persistence:** SwiftData for saving and loading user routes locally on device.
